...
import { phaseTargets } from "@/app/data";
...
/* ─── Build system prompt ─── */
function buildPrompt(cow: Cow, ingredients: Ingredient[]): string {
  const n = calcNutrition(cow.diet, ingredients);
...
export default function AIAnalysis({ cow, onClose, onApplyDiet }: Props) {
  ...
  const beforeNutr = calcNutrition(cow.diet, ingredients);
  const afterNutr  = result ? calcNutrition(result.suggestedDiet.map(d => ({ ingredientName: d.ingredientName, qtyMN: d.qtyMN })), ingredients) : null;
  const t = phaseTargets[cow.phase] ?? {};
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{ role: "user", content: buildPrompt(cow, ingredients) }],
        }),
      });
