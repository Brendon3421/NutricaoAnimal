...
import { phaseColors, type Cow, type DietLine, type HealthRecord } from "@/app/data";
import { useIngredients } from "@/app/ingredientStore";
import { ingredientCategories } from "@/app/ingredients";
...
export default function CadastroTab() {
  ...
  // ── Cálculo nutricional completo da dieta em edição (igual à aba Dieta da planilha) ──
  const dietLinesCalc = (form.diet ?? [])
    .map(d => {
      const ing = ingredients.find(i => i.name === d.ingredientName);
      return ing && d.qtyMN > 0 ? calcDietLine(ing, d.qtyMN) : null;
    })
    .filter((l): l is NonNullable<typeof l> => l !== null);
  const dietTotals = calcDietTotals(dietLinesCalc);
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ padding:0 }}>
            <div style={{ padding:"10px 12px", background:"#FFFBEB", borderBottom:"0.5px solid var(--border)", fontSize:11, color:"#854F0B" }}>
              💡 Adicione os ingredientes com as quantidades em <b>kg de matéria natural</b> por dia. O sistema calcula MS, NDT, PB e todos os outros nutrientes automaticamente a partir do cadastro de ingredientes.
            </div>
            ...
            <div style={{ padding:"10px 12px", borderTop:(form.diet??[]).length>0?"0.5px dashed var(--border)":"none", display:"flex", flexDirection:"column", gap:8 }}>
              ...
              {ingredients.length === 0 && (
                <div style={{ fontSize:11, color:"#A32D2D" }}>
                  Nenhum ingrediente cadastrado ainda. Vá até a aba <b>Ingredientes</b> para cadastrar.
                </div>
              )}
              <div style={{ display:"flex", gap:8 }}>
            ...
            {/* Status nutricional — total da dieta, calculado a partir do cadastro de ingredientes.
                Reativo: recalcula sozinho a qualquer troca de ingrediente, quantidade ou peso da vaca. */}
            {(form.diet??[]).length > 0 && (
              <StatusNutricionalGrid totals={dietTotals} pesoVaca={form.peso} phase={form.phase} cow={form} />
            )}
          </div>
        </div>

        <DetalhamentoNutricional lines={dietLinesCalc} totals={dietTotals} />
        </div>
      </div>
